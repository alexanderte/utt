...
(function() {
  var AppRouter, appRouter, homeView, iframeView, navbarView, resultView, testRun, testView;
  ...
  require(['jquery', 'underscore', 'backbone', 'tests', 'test-run', 'views/navbar', 'views/home', 'views/test', 'views/iframe', 'views/result'], function($, _, Backbone, Tests, TestRun, NavbarView, HomeView, TestView, IframeView, ResultView) {}, _.templateSettings = {
    interpolate: /\{\{(.+?)\}\}/g
  ...
  }), appRouter = new AppRouter(), navbarView = new NavbarView({
    model: testRun
  }), homeView = new HomeView(), testView = new TestView({
    model: testRun
  }), iframeView = new IframeView({
    model: testRun
  }), resultView = new ResultView(), navbarView.render(), iframeView.render(), appRouter.on('route:home', function() {
    homeView.render();
    return activateView('home');
  }), appRouter.on('route:test', function(id) {
    var _ref;
    testRun.set('currentTest', (_ref = id === void 0) != null ? _ref : {
      0: parseInt(id)
    });
    testView.render();
  ...
  }), Backbone.history.start(), activateView(name)(function() {
    switch (name) {
      case 'home':
